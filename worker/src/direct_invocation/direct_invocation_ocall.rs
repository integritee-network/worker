...
use crate::direct_invocation::watching_client::{WatchingClient, WsSend};
use crate::ocall_bridge::bridge_api::{
    DirectInvocationBridge, OCallBridgeError, OCallBridgeResult,
};
...
pub mod tests {
    ...
    fn test_continue_watching() {
        assert!(!DirectInvocationOCall::<()>::continue_watching(
            &TrustedOperationStatus::Invalid
        ));
        assert!(!DirectInvocationOCall::<()>::continue_watching(
            &TrustedOperationStatus::Usurped
