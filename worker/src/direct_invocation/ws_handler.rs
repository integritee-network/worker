...
use crate::direct_invocation::watching_client::{
    CreateWatchingClient, WatchingClient, WsSend, WsSender,
};
    ...
    Watch: WatchList<Client = Client>,
{
    fn on_message(&mut self, msg: Message) -> Result<()> {
        if self
            .handle_direct_invocation_request(msg.to_string().as_str())
            .is_err()
        {
...
mod tests {
    ...
    fn given_request_when_response_cannot_be_decoded_then_do_not_add_to_watch_list() {
        ...
        let invalid_response = RpcResponse {
