...
impl EnclaveAttestationOCallApi for OcallApi {
	...
	fn get_qve_report_on_quote(
		...
		let mut supplemental_data_size = std::mem::size_of::<sgx_ql_qv_supplemental_t>() as u32
		let mut supplemental_data = [0u8; supplemental_data_size];
		let mut qve_report_info_return_value: sgx_ql_qe_report_info_t = unsafe { std::mem::zeroed() };
		...
		let result = unsafe {
			ffi::ocall_get_qve_report_on_quote(
				...
				&app_enclave_target_info as *const sgx_target_info_t,
		...
		Ok(collateral_expiration_status, quote_verification_result, qve_report_info_return_value, supplemental_data.into_vec())
	}
