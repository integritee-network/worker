...
use ita_sgx_runtime::{AddressMapping, Index};
use ita_stf::{
	evm_helpers::{get_evm_account_codes, get_evm_account_storages},
...
pub fn test_evm_counter() {
	...
	// then
	let execution_address = H160::from_slice(
...
fn execute_and_verify_evm_call(
	...
	Stf::execute(state, inc_call, calls, [0u8, 1u8]).unwrap();
