...
use sp_core::{
    ...
    storage::ChildInfo,
    traits::{CallInWasmExt, KeystoreExt, TaskExecutorExt},
};
...
mod tests {
    ...
    fn dynamic_extensions_work() {
        ...
        assert!(ext
            .extensions()
            .get_mut(TypeId::of::<VerificationExt>())
            .is_some());
    ...
    fn long_sr25519_batching() {
        ...
        ext.execute_with(|| {
            ...
            // push invlaid
            crypto::sr25519_verify(&Default::default(), &Vec::new(), &Default::default());
    ...
    fn batching_works() {
        ...
        ext.execute_with(|| {
            ...
            crypto::start_batch_verify();
            crypto::ed25519_verify(&Default::default(), &Vec::new(), &Default::default());
            assert!(!crypto::finish_batch_verify());
