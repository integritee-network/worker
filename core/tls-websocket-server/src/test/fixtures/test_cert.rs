...
use std::{io::BufReader, vec::Vec};

pub fn get_test_certificate_chain() -> Vec<Certificate> {
	let mut buf_reader = BufReader::new(CERT_STR.as_bytes());
	certs(&mut buf_reader).unwrap()
}
