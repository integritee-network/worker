...
where
	...
	NodeMetadataProvider: AccessNodeMetadata<MetadataType = NodeMetadata>,
{
	pub fn new(
		shielding_key_repo: Arc<ShieldingKeyRepository>,
		stf_enclave_signer: Arc<StfEnclaveSigner>,
	...
	fn create_processed_parentchain_block_call(
		...
		let (teerex_module, processed_parentchain_block) =
			self.node_meta_data_provider.get_from_metadata(|meta_data| {
				(meta_data.teerex_module, meta_data.processed_parentchain_block)
			})?;

		let root: H256 = merkle_root::<Keccak256, _, _>(extrinsics).into();
		Ok(OpaqueCall::from_tuple(&(
			[teerex_module, processed_parentchain_block],
			block_hash,
			root,
		)))
	}
	...
	fn is_shield_funds_function(&self, function: &[u8; 2]) -> bool {
		self.node_meta_data_provider
			.get_from_metadata(|meta_data| {
				function == &[meta_data.teerex_module, meta_data.shield_funds]
			})
			.unwrap_or(false)
	}
	...
	fn is_call_worker_function(&self, function: &[u8; 2]) -> bool {
		self.node_meta_data_provider
			.get_from_metadata(|meta_data| {
				function == &[meta_data.teerex_module, meta_data.call_worker]
			})
			.unwrap_or(false)
	}
}
...
mod test {
	...
	fn ensure_empty_extrinsic_vec_triggers_zero_filled_merkle_root() {
		...
		let expected_call = (
			[dummy_metadata.teerex_module, dummy_metadata.processed_parentchain_block],
			block_hash,
			H256::default(),
		)
			.encode();
	...
	fn shield_funds_unchecked_extrinsic(
		...
		ParentchainUncheckedExtrinsic::<ShieldFundsFn>::new_signed(
			(
				[dummy_metadata.teerex_module, dummy_metadata.shield_funds],
				target_account,
				1000u128,
				shard_id(),
			),
