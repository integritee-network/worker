...
if tmux has-session -t integritee_logger_three_nodes ; then
  ...
  tmux new-session -d -s integritee_logger_three_nodes \; \
    ...
    send-keys -t integritee_logger_three_nodes:0.0 'tail -f ../log/node1.log' C-m \; \
    send-keys -t integritee_logger_three_nodes:0.1 'tail -f ../log/node2.log' C-m \; \
    send-keys -t integritee_logger_three_nodes:0.2 'tail -f ../log/node3.log' C-m \; \
    send-keys -t integritee_logger_three_nodes:0.3 'tail -f ../log/worker1.log' C-m \; \
