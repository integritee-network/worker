...
impl SetWinningsCommand {
    pub(crate) fn run(&self, cli: &Cli, trusted_args: &TrustedCli) -> CliResult {
        ...
        println!("send trusted call guess-the-number set winnings({}, {})", signer.public(), self.winnings);

        let (mrenclave, shard) = get_identifiers(trusted_args);
        let nonce = get_layer_two_nonce!(signer, cli, trusted_args);
        let top: TrustedOperation<TrustedCallSigned, Getter> = TrustedCall::guess_the_number_set_winnings(
            signer.public().into(),
            self.winnings,
        )
