...
pub struct MintCommand {
	/// sender's AccountId in ss58check format, mnemonic or hex seed. Must be admin of class id
	admin: String,
	/// class id
	id: u32,
...
impl MintCommand {
	pub(crate) fn run(&self, cli: &Cli, trusted_args: &TrustedCli) -> CliResult {
		...
		println!("send trusted call credits mint");

		let nonce = get_trusted_account_info(cli, trusted_args, &sender, &signer)
			.map(|info| info.nonce)
			.unwrap_or_default();

		let top: TrustedOperation<TrustedCallSigned, Getter> = TrustedCall::credits(
			CreditsTrustedCall::mint(sender, self.id, owner, self.amount, self.expiry),
		)
