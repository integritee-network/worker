...
use substratee_stf::{TrustedCallSigned, TrustedGetterSigned};
...
pub fn user_to_pubkey(user: &str) -> ed25519::Public {
    ed25519::Public::from_string(user)
        .ok()
        .or_else(|| {
            ed25519::Pair::from_string(user, Some(""))
                .ok()
                .map(|p| p.public())
        })
    ...
    MultiSignature: From<P::Signature>,
{
    ...
    let result_str = api
        .get_storage(category, item, Some(accountid.encode()))
        .unwrap();
...
pub fn get_wasm_hash(path: &str) -> Vec<String> {
    ...
    std::str::from_utf8(&sha_cmd.stdout)
        .unwrap()
        .split("  ")
        .map(|s| s.to_string())
        .collect()
