...
#[tokio::main]
async fn write_to_ipfs(client: &IpfsClient, data: &'static [u8], log_dir: Arc<Path>) -> OCallBridgeResult<IpfsCid> {
...
fn log_failing_blob_to_file(
    ...
    let cid_str = IpfsCid::from_content_bytes(&blob).map(|cid| format!("{}", cid)).unwrap_or("invalid-cid".to_string());
