	...
	fn get_dcap_collateral(&self, fmspc: Fmspc) -> EnclaveResult<*const sgx_ql_qve_collateral_t> {
		...
		unsafe {
			...
			trace!(
				...
				collateral.version,
				collateral.tee_type,
				std::ffi::CStr::from_ptr(collateral.pck_crl_issuer_chain).to_string_lossy().unwrap(),
				collateral.pck_crl_issuer_chain_size,
				std::ffi::CStr::from_ptr(collateral.root_ca_crl).to_string_lossy().unwrap(),
				collateral.root_ca_crl_size,
				std::ffi::CStr::from_ptr(collateral.pck_crl).to_string_lossy().unwrap(),
				collateral.pck_crl_size,
				std::ffi::CStr::from_ptr(collateral.tcb_info_issuer_chain).to_string_lossy().unwrap(),
				collateral.tcb_info_issuer_chain_size,
				std::ffi::CStr::from_ptr(collateral.tcb_info).to_string_lossy().unwrap(),
				collateral.tcb_info_size,
				std::ffi::CStr::from_ptr(collateral.qe_identity_issuer_chain).to_string_lossy().unwrap(),
				collateral.qe_identity_issuer_chain_size,
				std::ffi::CStr::from_ptr(collateral.qe_identity).to_string_lossy().unwrap(),
